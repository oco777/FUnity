...
| ピッチの効果を○ずつ変える | FUnity.Runtime.Integrations.VisualScripting.Units.ScratchUnits.ChangePitchEffectByUnit | ピッチの効果を増減 | FUnity/Blocks/音 | 半音単位でピッチ効果を相対変更し、AudioSource.pitch に反映。定義: Runtime/Integrations/VisualScripting/Units/ScratchUnits/SoundEffectUnits.cs |
| ピッチの効果を○にする | FUnity.Runtime.Integrations.VisualScripting.Units.ScratchUnits.SetPitchEffectToUnit | ピッチの効果を指定 | FUnity/Blocks/音 | 半音単位でピッチ効果を絶対設定し、即時適用。定義: Runtime/Integrations/VisualScripting/Units/ScratchUnits/SoundEffectUnits.cs |
| 左右にパンの効果を○ずつ変える | FUnity.Runtime.Integrations.VisualScripting.Units.ScratchUnits.ChangePanEffectByUnit | 左右にパンの効果を増減 | FUnity/Blocks/音 | 左右パン効果（%-100～100）を相対変更し、panStereo に反映。定義: Runtime/Integrations/VisualScripting/Units/ScratchUnits/SoundEffectUnits.cs |
| 左右にパンの効果を○にする | FUnity.Runtime.Integrations.VisualScripting.Units.ScratchUnits.SetPanEffectToUnit | 左右にパンの効果を指定 | FUnity/Blocks/音 | 左右パン効果を絶対設定し、AudioSource へ即時反映。定義: Runtime/Integrations/VisualScripting/Units/ScratchUnits/SoundEffectUnits.cs |
| 音の効果をなくす | FUnity.Runtime.Integrations.VisualScripting.Units.ScratchUnits.ClearSoundEffectsUnit | 音の効果をなくす | FUnity/Blocks/音 | ピッチ・パン効果を初期状態へ戻す。定義: Runtime/Integrations/VisualScripting/Units/ScratchUnits/SoundEffectUnits.cs |
...
| Scratch ブロック (日本語) | FUnity Unit クラス | UnitTitle | UnitCategory | 備考 |
| --- | --- | --- | --- | --- |
| 緑の旗が押されたとき | FUnity.Runtime.Integrations.VisualScripting.Units.ScratchUnits.WhenGreenFlagClickedUnit | 緑の旗が押されたとき | Events/FUnity/Blocks/イベント | Runner 対象の緑の旗イベント。EventBus 登録時に Flow を生成し、`flow.StartCoroutine(trigger)` で開始してから `ScratchUnitUtil.RegisterScratchFlow` でスレッド登録する。Flow の解放は Visual Scripting のコルーチン側に委任。定義: Runtime/.../GreenFlagUnits.cs |
| このスプライトが押されたとき | FUnity.Runtime.Integrations.VisualScripting.Units.ScratchUnits.WhenThisSpriteClickedUnit | このスプライトが押されたとき | Events/FUnity/Blocks/イベント | クリックされた俳優自身（Graph の self）だけを EventHook の target にして EventBus.Trigger を呼び出し、`flow.StartCoroutine(trigger)` → `ScratchUnitUtil.RegisterScratchFlow` で Scratch スレッド登録する。定義: Runtime/.../WhenThisSpriteClickedUnit.cs / クリック発火: Runtime/Core/FUnityManager.cs |
| ○キーが押されたとき | FUnity.Runtime.Integrations.VisualScripting.Units.ScratchUnits.OnKeyPressedUnit | ○キーが押されたとき | Events/FUnity/Blocks/イベント | FUnityManager.Update → `ScratchInputEventDispatcher.Tick()` 経由で毎フレーム呼び出される静的ディスパッチに登録し、押下エッジ検出時だけ Flow を生成して `flow.StartCoroutine(trigger)` を開始する。直後に `ScratchUnitUtil.RegisterScratchFlow` で登録し、停止ブロックと同期させる。EventBus には依存しない。定義: Runtime/.../InputEventUnits.cs |
| メッセージを送る | FUnity.Runtime.Integrations.VisualScripting.Units.ScratchUnits.BroadcastMessageUnit | メッセージを送る | FUnity/Blocks/イベント | 即時配信。定義: Runtime/.../MessagingUnits.cs |
| メッセージを送って待つ | FUnity.Runtime.Integrations.VisualScripting.Units.ScratchUnits.BroadcastAndWaitUnit | メッセージを送って待つ | FUnity/Blocks/イベント | 同期配信後に継続。定義: Runtime/.../MessagingUnits.cs |
| メッセージを受け取ったとき | FUnity.Runtime.Integrations.VisualScripting.Units.ScratchUnits.WhenIReceiveMessageUnit | メッセージを受け取ったとき | Events/FUnity/Blocks/イベント | フィルタ一致時に Flow を生成し、`flow.StartCoroutine(trigger)` で実行した直後に `ScratchUnitUtil.RegisterScratchFlow` で登録する。Flow の寿命は Visual Scripting のコルーチン管理に任せる。停止ブロックとの連動を担保。定義: Runtime/.../MessagingUnits.cs |
...
| 〇色に触れた？ | FUnity.Runtime.Integrations.VisualScripting.Units.ScratchUnits.TouchingColorPredicateUnit | 〇色に触れた？ | FUnity/Blocks/調べる | 背景色をグリッドサンプリングし、許容色差で一致を判定。定義: Runtime/.../TouchingColorPredicateUnit.cs |
...
| 答え | FUnity.Runtime.Integrations.VisualScripting.Units.Blocks.AnswerUnit | 答え | FUnity/Blocks/調べる | AnswerStore.LastAnswer に保持された直近の回答を文字列/float/int で返す。数値変換は float.TryParse と Mathf.RoundToInt を使用。定義: Runtime/Integrations/VisualScripting/Units/Blocks/QuestionUnits.cs |
...
| ○ = ○(整数) | FUnity.Runtime.Integrations.VisualScripting.Units.ScratchUnits.EqualIntegersUnit | ○ = ○(整数) | FUnity/Blocks/演算 | 2 つの整数が等しいかを判定する。定義: Runtime/Integrations/VisualScripting/Units/ScratchUnits/LogicOperatorUnits.cs |
| ○ = ○(文字) | FUnity.Runtime.Integrations.VisualScripting.Units.ScratchUnits.EqualStringsUnit | ○ = ○(文字) | FUnity/Blocks/演算 | 2 つの文字列が完全一致するかを判定する（大文字小文字を区別）。定義: Runtime/Integrations/VisualScripting/Units/ScratchUnits/LogicOperatorUnits.cs |
...
| 変数（変数）を隠す | FUnity.Runtime.Integrations.VisualScripting.Units.ScratchUnits.HideVariableUnit | 変数（変数）を隠す | FUnity/Blocks/変数 | 変数モニターを非表示。定義: Runtime/.../ScratchVariables/HideVariableUnit.cs |
