...
### 見た目: コスチューム切り替えの例
1. `FUnityActorData` の `Sprites` リストへ歩きモーションなどの差分 Sprite を Scratch で見せたい順番に登録します。リスト先頭が Scratch コスチューム番号 1 になります。
2. Visual Scripting で `ずっと` の中に `次のコスチュームにする` と `0.1 秒待つ` を配置し、ループごとに切り替えるグラフを組みます。
3. ゲームを再生するとコスチュームが 1 → 2 → … → N → 1 と循環し、歩きアニメーションが実現します。

> Scratch の UI に表示される番号は 1 始まりですが、内部では `ActorState.CostumeIndex` として 0 始まりで管理されます。Presenter が `ActorView.SetSprite(Sprite)` を通じて表示を更新するため、クローンを含むすべての俳優で状態が一貫します。
